...
xcrun safari-web-extension-converter dist/browser/safari --project-location /tmp/websuddhi-safari --copy-resources
# Opens in Xcode — click "Run" to install
```

The generated Xcode project is disposable build output. Re-run the converter whenever you need a fresh Safari package.

Or manually: Safari > Settings > Extensions > enable WebSuddhi.

### Safari (iOS / iPadOS)

1. Run `npm run build:targets`
2. Package `dist/browser/safari` with `xcrun safari-web-extension-converter --ios-only ...`
3. Open the generated Xcode project, connect your device, and click **Run**
...
| Social Blocker | OFF | Hides social login and social sharing widgets |
...
Safari packaging now comes from the same shared source tree via `npm run build:targets`, with a generated Safari manifest that strips unsupported permissions for Apple platforms.
...
3. Re-run `npm run build:targets` so the Safari bundle stays in sync
...
- Re-run `npm run build:targets` before testing Safari
...
- **Safari**: Run `npm run build:targets`, then package `dist/browser/safari` with `xcrun safari-web-extension-converter`
</details>
